...
var handleStates = require('./handleStates');
...
app.get('/api/calcPaycheck', function (request,response){
	...
	async.auto({
		...
		getStateTax:function (callback, results){
			...
			handleStates.handleState(stateInfo, callback);
		},
		...
		function calcTakehome (err, results){
			...
			var takehomePay = (income/payPeriods)-retirementContribution-fedTax-ssTax-medTax-stateTax;

			responseText = {
				fedAGI: accounting.formatMoney(results.getDedExempt.fedAGI),
...
app.get('/paycheck', function (request, response){
	async.parallel([
		function getListofFilingStatuses(callback){
			getFilingStatuses(callback);
		},
		function getListofStates(callback){
			getJurisdictions('State',callback);
		},
		function getListofPayFrequencies(callback){
			getPayFrequencies(callback);
		}
	],
		function createPaycheckCalculatorPage(err,results){
